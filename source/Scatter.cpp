...
SimState init_particles(int Nparticles, SimParams* p)
{
    ...
    result.rand_states = (RandState*)calloc(Nparticles, sizeof(RandState));

    // NOTE(cmo): Seed states
    result.rand_states[0] = Rand::seed_state(p->seed);
    for (int i = 1; i < Nparticles; ++i)
        result.rand_states[i] = Rand::copy_jump(&result.rand_states[i-1]);
...
void advance_dtsave(SimParams p, SimState* state)
{
    ...
    while (state->time - time0 < dt)
    {
        ...
        for (int i = 0; i < Nparticles; ++i)
        {
            ...
            auto res0 = draw_2_n(&state->rand_states[i]);
            auto res1 = draw_2_n(&state->rand_states[i]);
